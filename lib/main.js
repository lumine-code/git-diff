...
module.exports = {
  activate(_state) {
    ...
    // A diff view exists only for an editor in a repository, so a command that
    // needs one says as much rather than doing nothing.
    const forDiffView = (method) => (event) => {
      const editor = editorForEvent(event);
      const diffView = editor && viewsByEditor.get(editor);
      if (!diffView) {
        atom.notifications.addWarning("git-diff: no diff for this editor", {
    ...
    subscriptions.add(
      ...
      atom.commands.add("atom-workspace", {
        ...
        "git-diff:move-to-next-diff": forDiffView("moveToNextDiff"),
        "git-diff:move-to-previous-diff": forDiffView("moveToPreviousDiff"),
      }),
