...
module.exports = class GitDiffView {
   ...
   */
  destroyChildren() {
    if (this._animationId) this._animationWindow?.cancelAnimationFrame(this._animationId);
  ...
  scheduleUpdate() {
    ...
    this._animationWindow = this.editorElement.ownerDocument.defaultView;
    this._animationId = this._animationWindow.requestAnimationFrame(this.updateDiffs);
  }
  ...
  beginWindowSurfaceTransition() {
    ...
    this._animationWindow = null;
    const finish = () => {
      if (!this.destroyed) this.scheduleUpdate();
    };
